...
highlight <- function(
  ...
  highlighted_text <- c(
    if (!is.null(renderer$header)) renderer$header(),
    .Call(
      ...
      renderer$newline(),
      if (showPrompts) {
        renderer$formatter(renderer$translator(prompt, size = size), "prompt")
      } else {
        ""
      },
      if (showPrompts) {
        renderer$formatter(renderer$translator(continue, size = size), "prompt")
      } else {
        ""
      },
